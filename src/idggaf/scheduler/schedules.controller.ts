import { Controller, Get, Post, Put, Delete, Param, Res, Body, Query } from '@nestjs/common';
...
export class SchedulesController {
    ...
    async schedules(
        ...
        try {
            ...
            if (_id && !isNaN(id!))
                return res.status(200).json({ result: schedules.find(s => s.id === id) });
            else
                return res.status(200).json({ results: schedules });
    ...
    async addSchedule(
        ...
        try {
            ...
            const schedule = await scheduler.makeSchedule(new Date(body.toPostAt), body.content, body.webhook, body.details);
