...
export class FetcherController {
    ...
    async job(
        ...
        try {
            ...
            if (raw_page && isNaN(page as number)) throw new BadRequestException('Invalid page!');

            const num_pages = raw_num_pages ? parseInt(raw_num_pages) : undefined;

            if (raw_num_pages && isNaN(num_pages as number)) throw new BadRequestException('Invalid num page!');

            const radius = raw_radius ? parseInt(raw_radius) : undefined;

            if (raw_radius && isNaN(radius as number)) throw new BadRequestException('Invalid radius!');
    ...
    async reddit(
        ...
        try {
            ...
            if (limit !== undefined && isNaN(limit)) throw new BadRequestException('Invalid limit!');
    ...
    @Get('redgifs/hasId')
    redgifsHasId(@Query('url') url: string, @Res() res: Response) {
        try {
            if (!url) throw new BadRequestException('No url!');
    ...
    async redgifsGifs(
        ...
        try {
            ...
            if (ids.length <= 0 && !niche) throw new BadRequestException('No id or niche!');
                ...
                const { gifs: results, error } = await Redgifs.getNicheGIFs(
                    niche as Redgifs.NicheType,
                    hasAudio && hasAudio === 'true' ? Redgifs.hasAudioCriteria : () => true,
                );
